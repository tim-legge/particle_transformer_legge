...
# %%
MoE_model_seed_0 = get_model('jc_full')[0]
MoE_model_seed_1 = get_model('jc_full')[0]
MoE_model_10_pct = get_model('jc_full')[0]

model_path_seed_0 = f'models/jc_100_seed_0_net_epoch_state.pt'
data_dir_seed_0 = f'/moe-interpretability-pv/moe_stacked_bars_100k_data_seed_0_ll/'

model_path_seed_1 = f'models/jc_100_seed_1_net_epoch_state.pt'
data_dir_seed_1 = f'/moe-interpretability-pv/moe_stacked_bars_100k_data_seed_1_ll/'

model_path_10_pct = 'net_best_epoch_state.pt'
data_dir_10_pct = f'/moe-interpretability-pv/moe_stacked_bars_100k_data_ll/'
attempt = 0

MoE_statedict_seed_0 = torch.load(model_path_seed_0, map_location=torch.device('cpu'))
MoE_model_seed_0.load_state_dict(MoE_statedict_seed_0)

MoE_statedict_seed_1 = torch.load(model_path_seed_1, map_location=torch.device('cpu'))
MoE_model_seed_1.load_state_dict(MoE_statedict_seed_1)

MoE_statedict_10_pct = torch.load(model_path_10_pct, map_location=torch.device('cpu'))
MoE_model_10_pct.load_state_dict(MoE_statedict_10_pct)

# %%
with open(data_dir_seed_0+'counter_stacked_MoE_bars_100k.txt', 'r') as f:
    start_counter_seed_0 = int(f.read().strip())
with open(data_dir_seed_1+'counter_stacked_MoE_bars_100k.txt', 'r') as f:
    start_counter_seed_1 = int(f.read().strip())
with open(data_dir_10_pct+'counter_stacked_MoE_bars_100k.txt', 'r') as f:
    start_counter_10_pct = int(f.read().strip())
...
start_counter = min(start_counter_seed_0, start_counter_seed_1, start_counter_10_pct)
...
while start_counter < 100000//howmanyjets:
    ...
    for model, data_dir in zip([MoE_model_seed_0, MoE_model_seed_1, MoE_model_10_pct],
                               [data_dir_seed_0, data_dir_seed_1, data_dir_10_pct]):
        router_hook = Router_Hook(model=model)
        model.eval()
        with torch.no_grad():
            y_pred = MoE_model_seed_0(torch.from_numpy(points),torch.from_numpy(features),
