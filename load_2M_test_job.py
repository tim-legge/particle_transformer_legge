...
def load_data(file, dataset_type='jc_full', start_index=None, batch_size=None):
    """
    Load data from 
    ...
    try:
        if dataset_type == 'qg':
            ...
            data_path = file
            if os.path.exists(data_path):
                print(f"Loading JetClass data from {data_path}")
                with uproot.open(data_path)['tree'] as tree:
...
test_set_root_dir = '/moe-interpretability-pv/datasets/2M_test/'
...
print('Downloaded JC dataset!')

np.save(file_to_load.replace('.root', '_points.npy'), jc_full_data['pf_points'])
np.save(file_to_load.replace('.root', '_features.npy'), jc_full_data['pf_features'])
np.save(file_to_load.replace('.root', '_vectors.npy'), jc_full_data['pf_vectors'])
